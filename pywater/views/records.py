...
class WeightRecordApp(QWidget):
    ...
    def plot_chart(self):
        ...
        plt.plot(self.loaded_data['日期'], self.loaded_data['體重'], linestyle='-', marker='o', label='體重趨勢', color='blue')
        plt.title('體重變化折線圖')
        plt.xlabel('日期')
        plt.ylabel('體重 (kg)')
        plt.legend()
    ...
    def add_data(self):
        date_input, ok = QLineEdit.getText(QInputDialog(), '新增數據', '請輸入要新增的日期(如：2023-12-01):')
        weight_input, ok = QLineEdit.getText(QInputDialog(), '新增數據', '請輸入體重，單位為kg(如：62.1):')
        ...
        try:
            ...
            if date not in self.loaded_data['日期'].values:
                ...
                self.loaded_data = pd.concat([self.loaded_data, new_data], ignore_index=True)
    ...
    def modify_data(self):
        date_input, ok = QLineEdit.getText(QInputDialog(), '修改特定紀錄', '請輸入要修改的日期(如：2023-12-01):')
        ...
        try:
            ...
            if date in self.loaded_data['日期'].values:
                ...
                new_weight, ok = QLineEdit.getText(QInputDialog(), '修改特定紀錄', '請輸入新的體重，單位為kg:')
    ...
    def delete_data(self):
        date_input, ok = QLineEdit.getText(QInputDialog(), '刪除特定紀錄', '請輸入要刪除的日期(如：2023-12-01):')
